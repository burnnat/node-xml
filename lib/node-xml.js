...
XMLP.prototype._parseDTD = function(iB) {
    ...
    this._setContent(XMLP._CONT_ALT, this.m_xml.substring(iB, this.m_iP - 1));

    return XMLP._DTD;

}

XMLP.prototype._parseDoctype = function(contents) {
    var pieces = contents.match(/(?:[^\s"]+|"[^"]*")+/g);
    var data = { root: pieces[0] };
    
    if (pieces[1] === 'SYSTEM') {
        data.systemId = pieces[2];
    }
    else if (pieces[1] === 'PUBLIC') {
        data.publicId = pieces[2];
        data.systemId = pieces[3];
    }
...
var SaxParser = function(eventhandlerfactory) {
    ...
    thehandler.prototype.onDTD = function (funct){
      eventhandler.onDTD = funct;
    }
...
SaxParser.prototype._fireEvent = function(iEvt) {
    ...
    else if (SaxParser.DTD   == iEvt) {
        func = "onDTD";                   hnd = this.m_hndDoc;
    }
...
SaxParser.prototype._parseLoop = function(parser) {
    ...
    while(!this.m_bErr) {
        ...
        if(iEvent == XMLP._ELM_B) {
            ...
            var doctype = parser._parseDoctype(trim(parser.getContent()));
            this._fireEvent(SaxParser.DTD, doctype.root, doctype.publicId || null, doctype.systemId || null);
        }
